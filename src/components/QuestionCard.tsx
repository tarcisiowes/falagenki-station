...
export function QuestionCard({ q, furigana, onFinish, finishLabel = 'Próximo exercício' }: QuestionCardProps) {
  ...
  const [noteOpen, setNoteOpen] = useState(() => Boolean(rec?.note))
  ...
  return (
    <div className="card q" id={q.id} tabIndex={-1}>
      <div className="qhead q-title-row">
        <span className="qnum">{q.number}</span>
        <div className="q-title-content">
          {q.context && (
          ...
          <div className="stem ja">
            ...
            {q.helpPt && (
              <StudyHelp variant="popover">
                <StudyText text={q.helpPt} furigana={furigana} />
              </StudyHelp>
            )}
          </div>
        </div>
      ...
      <div className="qbody">
        ...
        <div className="question-action-row">
          ...
          >
            {checked
              ? 'Ocultar'
              : (isSelfCheck ? 'Registrar' : 'Verificar')}
          </button>
          {onFinish && (
            <button className="btn small" type="button" onClick={onFinish} disabled={!checked}>
        ...
        >
          <summary><NotebookPen size={14} /> Anotação <span>opcional, salva automaticamente</span></summary>
          <div className="note-field">
            <label htmlFor={`${q.id}-note`}>Sua anotação ou justificativa</label>
            <textarea
              id={`${q.id}-note`}
              value={rec?.note ?? ''}
